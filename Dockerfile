...
WORKDIR /src
COPY ["image-board-backend.csproj", "."]
RUN dotnet restore "./image-board-backend.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./image-board-backend.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./image-board-backend.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "image-board-backend.dll"]